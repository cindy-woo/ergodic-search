...
def optimize_trajectory(x0, phik, k_expanded, lamk, lambda_goal, hk, info_map, u_init=None, T = 100, num_iters=1500, lr=0.001):
...
for i, info_map in enumerate(maps):
    ...
    optimized_u = optimize_trajectory(x0, phik, k_expanded, lambda_goal, lamk, hk, info_map)
    ...
    plt.scatter(executed_traj[1:, 0], executed_traj[1:, 1], s=10, c = 5 * torch.sigmoid(5 * optimized_u[:, 2]), cmap='plasma')
