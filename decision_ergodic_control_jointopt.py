...
def replanning(maps, _s, k_expanded, lamk, num_iters=1500):
    ...
    for i, info_map in enumerate(maps):
        ...
        u_optimized, loss= optimize_trajectory(x0, phik, k_expanded, lamk, hk, info_map, u_prev = u_prev, tau = tau, num_iters=num_iters)
...
trajectory, loss_list = replanning(maps, _s, k_expanded, lamk)

with xlsxwriter.Workbook('loss.xlsx') as workbook:
    worksheet = workbook.add_worksheet()
    for row_num, data in enumerate(loss_list):
