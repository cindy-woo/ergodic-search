...
def fingerprint_map(map):
    return (float(map.mean()), float(map.std()), float(map.min()), float(map.max()))
...
def replanning(maps, _s, k_expanded, lamk, num_iters=1500):
    ...
    for i, info_map in enumerate(maps):
        ...
        fingerprint = fingerprint_map(info_map)
        print(f"\n=== cycle {i} ===")
        print("map fingerprint ", fingerprint)
        ...
        # if 'info_map_prev' in locals():
        #     plt.figure(); plt.title("Map difference")
        #     plt.imshow((info_map - info_map_prev).numpy(), origin='lower', cmap='bwr'); plt.colorbar()
        # info_map_prev = info_map.clone()
