...
def extract_hotspots(info_map_norm, max_hotspots=MAX_HOTSPOTS, hotspot_quantile=HOTSPOT_QUANTILE, min_sep=HOTSPOT_MIN_SEP):
    ...
    device = info_map_norm.device
    
    # 1. Create a binary mask of "high info" pixels
    thr = torch.quantile(info_map_norm.flatten(), hotspot_quantile)
    binary_mask = (info_map_norm >= thr).cpu().numpy()
    
    # 2. Identify connected clusters (blobs)
    labeled_array, num_features = label(binary_mask)
    
    blob_candidates = []
    for i in range(1, num_features + 1):
        # Calculate area (number of pixels) in this blob
        coords = np.argwhere(labeled_array == i)
        area = len(coords)
        
        # ONLY consider the blob if it's larger than our threshold
        if area >= MIN_HOTSPOT_AREA:
            # Find the "peak" pixel inside this specific blob
            blob_mask = torch.from_numpy(labeled_array == i).to(device)
            blob_values = info_map_norm * blob_mask
            
            max_val = torch.max(blob_values)
            idx_i = int(torch.argmax(blob_values.flatten()).item())
            
            y = idx_i // w
            x = idx_i % w
            pos = torch.tensor([x / (w - 1), y / (h - 1)], dtype=torch.float32, device=device)
            
            blob_candidates.append({'pos': pos, 'score': max_val, 'area': area})

    # 3. Sort candidates by intensity (or area, if preferred)
    blob_candidates = sorted(blob_candidates, key=lambda x: x['score'], reverse=True)
    ...
    # 4. Fallback: If no large blobs exist, pick the absolute global maximum pixel
    if not hotspots:
        idx_i = int(torch.argmax(info_map_norm).item())
        y = idx_i // w
        x = idx_i % w
        hotspots = [torch.tensor([x / (w-1), y / (h-1)], dtype=torch.float32, device=device)]
        scores = [torch.max(info_map_norm)]
